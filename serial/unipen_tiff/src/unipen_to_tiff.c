...
#define FILE_PATH         "../input/pendigits-orig.tes"
...
void write_file(unsigned char *buffer, unsigned int imageNumber,
	...
	char fileName[50];
	sprintf(fileName, "../output/image%d.tiff", imageNumber);
...
void generate_tiff_file(struct header header, struct tag tags[11],
	...
	//copy header
	my_memcpy(marker, &header, sizeof(struct header));
	marker += sizeof(struct header);
	//copy pixel data
	int i;
	for(i = 0; i < PIXEL_DIMMENSION; ++i) {
		my_memcpy(marker, pixelDataBuffer[i], PIXEL_DIMMENSION);
		marker += PIXEL_DIMMENSION;
	}
	//copy TIFF footer
	my_memcpy(marker, &numTags, sizeof(numTags));
	marker += sizeof(numTags);
	//copy all tags

	for(i = 0; i < numTags; ++i){
		my_memcpy(marker, &tags[i], sizeof(struct tag));
		marker += sizeof(struct tag);
	}
	//copy end bytes
	my_memcpy(marker, &tiffEnd, sizeof(tiffEnd));
